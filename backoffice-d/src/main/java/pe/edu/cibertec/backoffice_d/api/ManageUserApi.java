    ...
    public GetAllUsersResponse getAllUsers() {
        try {
            ...
            if (!users.isEmpty())
                ...
                return new GetAllUsersResponse("02", "User not found", null);


        } catch (Exception e) {
            e.printStackTrace();
            return new GetAllUsersResponse("99", "An error ocurred, please try again", null);
        }
    }


    @GetMapping("/detail")
    public GetDetailUsersResponse getDetailUsers(@RequestParam(value = "id", defaultValue = "0") String id) {

        try {
            Optional<UserDetailDto> optional = manageService.getDetailUser(Integer.parseInt(id));
            return optional.map(user->
                    new GetDetailUsersResponse("01", null, user)
            ).orElse(
                    new GetDetailUsersResponse("02", "Usernot found", null)
            );
        } catch (Exception e) {
        e.printStackTrace();
        return new GetDetailUsersResponse("99", "An error ocurred, please try again", null);
        }
    ...
    public DeleteUserResponse deleteUser(@PathVariable String id){
        ...
        try {
            if(manageService.deleteUser(Integer.parseInt(id)))
                ...
                return new DeleteUserResponse("02", "Update failed");
    ...
    public CreateUserResponse createUser(@RequestBody UserCreateDto userCreateDto){
        ...
        try {
            if(manageService.createUser(userCreateDto))
                ...
                return new CreateUserResponse("02", "Update failed");
